...
class GlobalRegionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $globalRegions = GlobalRegion::query()->when($search, fn($q) => $q->where(array_key_first((new GlobalRegion)->getFillable() ? array_flip((new GlobalRegion)->getFillable()) : []), 'like', "%{$search}%"))->latest()->paginate(15)->withQueryString();
    ...
    {
        $dynamicData = [];
        return view('generated/global-regions.create', $dynamicData);
    }
    public function store(Request $request)
    {
        $data = $request->validate([
            'global_region_name' => ['required', 'string'],
            'global_region_code' => ['nullable', 'string'],
        ]);
        $globalRegion = GlobalRegion::create($data);
    ...
    public function edit(GlobalRegion $globalRegion)
    {
        $dynamicData = [];
        return view('generated/global-regions.edit', array_merge(compact('globalRegion'), $dynamicData));
    }
