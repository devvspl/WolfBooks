...
<div class="bg-white border border-stone-200 rounded-2xl overflow-hidden">
    <div class="px-6 py-5 border-b border-stone-100 flex items-center justify-between">
        <div>
            <h3 class="text-sm font-semibold text-stone-800">Edit Global Region</h3>
    ...
    <form method="POST" action="{{ route('generated.global-regions.update', $globalRegion) }}" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="p-6">
            @if($errors->any())
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="global_region_name" value="{{ $globalRegion->global_region_name }}" placeholder="Global Region Name" class="w-full px-3.5 py-2.5 text-sm border rounded-xl outline-none transition border-stone-300 focus:border-red-700 focus:ring-2 focus:ring-red-700/10 @error('global_region_name') border-red-400 bg-red-50 @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="global_region_code" value="{{ $globalRegion->global_region_code }}" placeholder="Global Region Code" class="w-full px-3.5 py-2.5 text-sm border rounded-xl outline-none transition border-stone-300 focus:border-red-700 focus:ring-2 focus:ring-red-700/10 @error('global_region_code') border-red-400 bg-red-50 @enderror">
                ...
                <div>
                    ...
                    <select name="new_option" class="w-full px-3.5 py-2.5 text-sm border rounded-xl outline-none transition border-stone-300 focus:border-red-700 focus:ring-2 focus:ring-red-700/10 @error('new_option') border-red-400 bg-red-50 @enderror"><option value="">-- Select --</option></select>
                    @error('new_option')<p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
            </div>
        </div>
