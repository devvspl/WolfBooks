...
@section('master-content')
    ...
    <div x-data="{
        ...
        <div class="bg-white border border-stone-200 rounded-2xl overflow-hidden">
            <div class="px-6 py-5 border-b border-stone-100 flex items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-stone-400 mt-0.5">{{ $pages->total() }}
                        {{ Str::plural('page', $pages->total()) }}</p>
                </div>
                <div class="flex items-center gap-3">
                    <form method="GET" action="{{ route('master.page-builder') }}">
                        <div
                            class="flex items-center gap-2 border border-stone-300 rounded-xl px-3 py-2
                            ...
                            @if($search)
                                <a href="{{ route('master.page-builder') }}"
                                    class="text-stone-400 hover:text-stone-600 transition shrink-0">
                    ...
                    <a href="{{ route('master.page-builder.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-800 hover:bg-red-700
            ...
            @if($pages->isEmpty())
                <div class="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p class="text-sm font-medium text-stone-600">
                        {{ $search ? 'No results for "' . $search . '"' : 'No pages yet' }}</p>
                    <p class="text-xs text-stone-400 mt-1">
                        {{ $search ? 'Try a different search term.' : 'Click "New Page" to get started.' }}</p>
                </div>
            @else
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-stone-100 bg-stone-50 text-left">
                            ...
                            <th class="px-6 py-3 text-xs font-semibold text-stone-500 tracking-wider">Page Name</th>
                            <th class="px-6 py-3 text-xs font-semibold text-stone-500 tracking-wider">Created</th>
                            <th class="px-6 py-3 text-xs font-semibold text-stone-500 tracking-wider">Updated</th>
                            <th class="px-6 py-3 text-xs font-semibold text-stone-500 tracking-wider text-right">
                                Actions</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-stone-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="inline-flex items-center gap-2">
                                        <a href="{{ route('master.page-builder.fields', $page) }}" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium
                                        ...
                                        <a href="{{ route('master.page-builder.edit', $page) }}" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium
                                        ...
                                        <form method="POST" action="{{ route('master.page-builder.destroy', $page) }}"
                                            ...
                                            @csrf @method('DELETE')
                                            <button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium
                    ...
                    <div class="px-6 py-4 border-t border-stone-100 flex items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-1">
                            @if($pages->onFirstPage())
                                <span
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-stone-300 cursor-not-allowed">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @else
                                <a href="{{ $pages->previousPageUrl() }}"
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-stone-500 hover:bg-stone-100 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                                    </svg>
                                </a>
                            ...
                            @foreach($pages->getUrlRange(1, $pages->lastPage()) as $pg => $url)
                                <a href="{{ $url }}"
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-xs font-medium transition-colors {{ $pg == $pages->currentPage() ? 'bg-red-800 text-white' : 'text-stone-600 hover:bg-stone-100' }}">{{ $pg }}</a>
                            @endforeach
        ...
        {{-- Offcanvas overlay --}}
        <div x-show="open" x-transition:enter="transition-opacity duration-200" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity duration-200"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black/40 z-40"
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-stone-200 shrink-0">
                <div>
                    ...
                    <p class="text-xs text-stone-400 mt-0.5"
                        x-text="fields.length + ' field' + (fields.length !== 1 ? 's' : '') + ' added'"></p>
                </div>
                    ...
                    <ul class="divide-y divide-stone-100 max-h-48 overflow-y-auto">
                        <template x-for="field in fields" :key="field.id">
                            <li class="flex items-center gap-3 px-5 py-2.5">
                                ...
                                <button @click="removeField(field.id)" class="w-7 h-7 flex items-center justify-center rounded-lg text-stone-400
                        ...
                        <div class="border border-stone-200 rounded-xl p-3 hover:border-stone-300 transition-colors">
                            <div class="flex items-center gap-2 mb-2.5">
                                <div class="w-7 h-7 rounded-lg bg-stone-100 flex items-center justify-center shrink-0">
                                    <svg class="w-3.5 h-3.5 text-stone-600" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                            ...
                            <div class="flex gap-1.5">
                                <input type="text" x-model="fieldInputs['{{ $type }}']"
                                    @keydown.enter.prevent="addField('{{ $type }}')" placeholder="Field name" class="flex-1 min-w-0 px-2.5 py-1.5 text-xs border border-stone-300 rounded-lg outline-none
