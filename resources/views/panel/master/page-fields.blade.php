...
<div
    ...
    style="height: calc(100vh - 120px); min-height: 0;"
>
    ...
    <div
        ...
        "
        :class="paletteOpen ? 'translate-y-0' : 'translate-y-full lg:translate-y-0'"
        style="height: auto;"
    ...
    >
        ...
        <div class="px-4 py-3 border-b border-white/10 shrink-0">
            <div class="flex items-center justify-between mb-2.5">
                ...
                <button @click="paletteOpen = false" class="lg:hidden w-6 h-6 flex items-center justify-center rounded-lg text-stone-400 hover:bg-white/10">
            ...
            <div class="relative">
                <svg class="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-stone-500 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
                <input type="text" x-model="search" placeholder="Search…"
                       class="w-full pl-7 pr-3 py-1.5 text-xs bg-white/10 border border-white/15 rounded-lg
        ...
        <div class="flex-1 overflow-y-auto p-2 space-y-0.5">
            ...
            <form method="POST" action="{{ route('master.page-builder.fields.store', $page) }}"
                ...
                <div class="group flex items-center gap-2.5 px-3 py-2 rounded-xl hover:bg-white/8 transition-colors cursor-pointer"
                    ...
                    <div class="w-7 h-7 rounded-lg bg-white/8 flex items-center justify-center shrink-0 group-hover:bg-red-800/60 transition-colors">
                        <svg class="w-3.5 h-3.5 text-stone-400 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input type="text" name="field_name" placeholder="Name"
                           ...
                           class="w-0 group-hover:w-20 opacity-0 group-hover:opacity-100 transition-all duration-200
    ...
    <div class="flex-1 min-w-0 flex flex-col bg-white border border-stone-200 rounded-2xl overflow-hidden">
        ...
        {{-- Toolbar --}}
        <div class="px-4 sm:px-5 py-3 sm:py-3.5 border-b border-stone-100 flex flex-wrap items-center justify-between gap-2 shrink-0">
            <div class="min-w-0">
        ...
        <div class="flex-1 overflow-y-auto p-4 sm:p-6">
            ...
            <div class="flex flex-col items-center justify-center h-full text-center py-20">
                ...
                <p class="text-xs text-stone-300 mt-1 max-w-xs">Pick a field type from the left panel and give it a name.</p>
                {{-- Mobile CTA --}}
            ...
            <div id="sortable-fields"
                ...
                <div class="group relative {{ $smSpan }} {{ $lgSpan }} field-item"
                     data-id="{{ $field->id }}">

                    {{-- Drag handle (visible in reorder mode) --}}
                    <div x-show="reorderMode"
                         class="drag-handle absolute -left-1 top-1/2 -translate-y-1/2 z-10 cursor-grab active:cursor-grabbing
                    ...
                    <div class="relative z-1 flex items-center justify-between mb-1">
                        <label class="text-xs font-medium text-stone-600 leading-tight">
                            ...
                            @if($field->is_unique)<span class="ml-1 text-[9px] font-semibold text-blue-500 bg-blue-50 px-1 py-0.5 rounded">U</span>@endif
                        </label>
                        <div class="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity"
                            ...
                            <form method="POST" action="{{ route('master.page-builder.fields.destroy', [$page, $field]) }}"
                                ...
                                @csrf @method('DELETE')
                                <button type="submit" class="w-6 h-6 flex items-center justify-center rounded text-stone-400 hover:text-red-500 hover:bg-red-50 transition-colors touch-manipulation">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                    </svg>
                                </button>
                        ...
                        <table class="w-full text-xs min-w-max">
                            <thead class="bg-stone-800 text-white">
                                <tr>
                                    ...
                                    @foreach($rCols as $rc)
                                    <th class="px-2 py-1.5 text-left font-medium whitespace-nowrap">{{ $rc['label'] }}@if(!empty($rc['required']))<span class="text-red-400 ml-0.5">*</span>@endif</th>
                                    @endforeach
                                ...
                                <tr class="bg-white">
                                    ...
                                    <td class="px-2 py-1.5">
                                        @if(in_array($rc['type'], ['select']))
                                            <select disabled class="w-full px-1.5 py-1 border border-stone-200 rounded bg-stone-50 text-stone-400 text-xs pointer-events-none"><option>—</option></select>
                                        @elseif($rc['type'] === 'checkbox')
                                            <input type="checkbox" disabled class="w-3.5 h-3.5 border-stone-300 pointer-events-none">
                                        @else
                                            <input type="text" disabled placeholder="{{ $rc['default'] ?? '' }}" class="w-full px-1.5 py-1 border border-stone-200 rounded bg-stone-50 text-stone-400 text-xs pointer-events-none">
                                        @endif
                                    </td>
                                    @endforeach
                                    <td class="px-2 py-1.5 text-center"><span class="inline-flex w-5 h-5 items-center justify-center rounded bg-green-600 text-white text-xs font-bold">+</span></td>
                                </tr>
                                <tr class="bg-stone-50/50">
                                    <td class="px-2 py-1.5 text-stone-400">2</td>
                                    @foreach($rCols as $rc)
                                    <td class="px-2 py-1.5"><input type="text" disabled class="w-full px-1.5 py-1 border border-stone-200 rounded bg-white text-stone-400 text-xs pointer-events-none"></td>
                                    @endforeach
                                    <td class="px-2 py-1.5 text-center"><span class="inline-flex w-5 h-5 items-center justify-center rounded bg-red-600 text-white text-xs font-bold">−</span></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    @elseif($field->field_type === 'content' || $field->field_type === 'json')
                        <textarea disabled placeholder="{{ $field->placeholder ?: ($field->label ?? $field->field_name) }}"
                    ...
                    @elseif($field->field_type === 'checkbox' || $field->field_type === 'toggle')
                        <div class="flex items-center gap-2 px-3 py-2 border border-stone-200 rounded-lg bg-stone-50 pointer-events-none">
                            ...
                            <span class="text-xs text-stone-400">{{ $field->placeholder ?: ($field->label ?? $field->field_name) }}</span>
                        </div>
                    @elseif($field->field_type === 'select')
                        <select disabled class="w-full px-3 py-2 text-xs border border-stone-200 rounded-lg bg-stone-50 text-stone-400 pointer-events-none">
                            <option>{{ $field->placeholder ?: '— Select —' }}</option>
                        </select>
                    @elseif(in_array($field->field_type, ['image', 'file']))
                        <div class="w-full px-3 py-2 text-xs border border-stone-200 rounded-lg bg-stone-50 text-stone-400 flex items-center gap-2 pointer-events-none">
                    ...
                    {{-- Hover ring --}}
                    <div class="absolute inset-0 rounded-lg pointer-events-none opacity-0 group-hover:opacity-100 ring-2 ring-red-700/25 transition-opacity" x-show="!reorderMode"></div>
                </div>
                @endforeach
            </div>
            @endif
    ...
    <div x-show="settingsOpen"
        ...
        <div class="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
            <div class="min-w-0">
                ...
                <h3 class="text-sm font-semibold text-white truncate" x-text="activeField?.label || activeField?.field_name"></h3>
            </div>
            <button @click="settingsOpen = false" class="w-7 h-7 flex items-center justify-center rounded-lg text-stone-400 hover:bg-white/10 hover:text-white transition-colors shrink-0 ml-3 touch-manipulation">
        ...
        <div class="flex-1 overflow-y-auto p-5">
            <template x-if="activeField">
                <div x-data="repeaterSettings(activeField)" class="space-y-4">
                    ...
                    <template x-if="activeField.field_type !== 'repeater'">
                        <form method="POST" :action="activeField.settings_url" class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-3 gap-2">
                                    <label class="cursor-pointer">
                                        <input type="radio" name="col_span" value="1" :checked="(activeField.col_span ?? 1) == 1" class="sr-only peer">
                                        <div class="flex flex-col items-center gap-1.5 px-3 py-2.5 rounded-lg border border-white/15 bg-white/5 peer-checked:border-red-500 peer-checked:bg-red-900/30 transition-colors text-center">
                                            <div class="flex gap-0.5 w-full"><div class="h-2 rounded-sm bg-red-500 w-1/3"></div><div class="h-2 rounded-sm bg-white/10 w-1/3"></div><div class="h-2 rounded-sm bg-white/10 w-1/3"></div></div>
                                            <span class="text-[10px] text-stone-400 font-medium">1 / 3</span>
                                        </div>
                                    </label>
                                    <label class="cursor-pointer">
                                        <input type="radio" name="col_span" value="2" :checked="(activeField.col_span ?? 1) == 2" class="sr-only peer">
                                        <div class="flex flex-col items-center gap-1.5 px-3 py-2.5 rounded-lg border border-white/15 bg-white/5 peer-checked:border-red-500 peer-checked:bg-red-900/30 transition-colors text-center">
                                            <div class="flex gap-0.5 w-full"><div class="h-2 rounded-sm bg-red-500 w-2/3"></div><div class="h-2 rounded-sm bg-white/10 w-1/3"></div></div>
                                            <span class="text-[10px] text-stone-400 font-medium">2 / 3</span>
                                        </div>
                                    </label>
                                    <label class="cursor-pointer">
                                        <input type="radio" name="col_span" value="3" :checked="(activeField.col_span ?? 1) == 3" class="sr-only peer">
                                        <div class="flex flex-col items-center gap-1.5 px-3 py-2.5 rounded-lg border border-white/15 bg-white/5 peer-checked:border-red-500 peer-checked:bg-red-900/30 transition-colors text-center">
                                            <div class="flex gap-0.5 w-full"><div class="h-2 rounded-sm bg-red-500 w-full"></div></div>
                            ...
                            <div>
                                ...
                                <div class="flex flex-wrap gap-4">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="is_required" value="1" :checked="activeField.is_required === 'true'" class="w-4 h-4 rounded border-white/20 bg-white/10 text-red-700 focus:ring-red-700 cursor-pointer">
                                        <span class="text-sm text-stone-300">Required</span>
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="is_unique" value="1" :checked="activeField.is_unique === 'true'" class="w-4 h-4 rounded border-white/20 bg-white/10 text-red-700 focus:ring-red-700 cursor-pointer">
                                        <span class="text-sm text-stone-300">Unique</span>
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="is_nullable" value="1" :checked="activeField.is_nullable === 'true'" class="w-4 h-4 rounded border-white/20 bg-white/10 text-red-700 focus:ring-red-700 cursor-pointer">
                            ...
                            <button type="submit" class="w-full py-2.5 rounded-xl bg-red-800 hover:bg-red-700 text-white text-sm font-semibold transition-colors">
                                Save Changes
                            </button>
                        </form>
                    </template>
                    ...
                    <template x-if="activeField.field_type === 'repeater'">
                        <div class="space-y-4">
                            <form method="POST" :action="activeField.settings_url" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                @csrf @method('PUT')
                                <div>
                                ...
                                <div>
                                    <label class="block text-xs font-medium text-stone-400 mb-1.5">Column Name (key)</label>
                                    <input type="text" name="column_name" :value="activeField.column_name"
                                           class="w-full px-3 py-2 text-sm bg-white/10 border border-white/20 rounded-lg outline-none text-white focus:border-red-500 transition">
                                </div>
                                ...
                                <div class="sm:col-span-2">
                                    <button type="submit" class="w-full py-2 rounded-lg bg-white/10 hover:bg-white/15 text-white text-xs font-medium transition-colors">
                                        Save Label / Key
                                    </button>
                                </div>
                            </form>
                            ...
                            <div class="border-t border-white/10 pt-4">
                                        ...
                                        <div class="bg-white/5 border border-white/10 rounded-xl p-3 space-y-2">
                                            ...
                                            <div class="grid grid-cols-2 gap-2">
                                                <div>
                                                    ...
                                                    <input type="text" x-model="col.label" @input="col.key = slugify(col.label)"
                                                ...
                                                <div>
                                                    <label class="block text-[10px] text-stone-500 mb-1">Key <span class="text-stone-600">(auto)</span></label>
                                                    <input type="text" x-model="col.key"
                                            ...
                                            <div class="grid grid-cols-2 gap-2">
                                                <div>
                                                    ...
                                                    <select x-model="col.type"
                                                            class="w-full px-2 py-1.5 text-xs bg-white/10 border border-white/15 rounded-lg outline-none text-white focus:border-red-500 transition">
                                                ...
                                                <div>
                                                    ...
                                                    <input type="text" x-model="col.default"
                                ...
                                <form method="POST" :action="activeField.repeater_url" class="mt-4" @submit.prevent="submitRepeater($event)">
                                    @csrf @method('PUT')
...
<script>
/* ─── Repeater settings component ─── */
function repeaterSettings(field) {
    return {
        columns: JSON.parse(JSON.stringify(field.repeater_columns || [])),
